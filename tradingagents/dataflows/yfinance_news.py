...
def get_global_news_yfinance(
    ...
    if config.get("market_scan_date") == curr_date:
        as_of = _as_utc(datetime.fromisoformat(config["market_scan_as_of"]))
    ...
    for query in search_queries:
        try:
            ...
            articles = [data for data in articles
                        if ((data["pub_date"] is None or start_dt <= _as_utc(data["pub_date"]) <= as_of)
                            if as_of else _in_news_window(data["pub_date"], start_dt, curr_dt))]

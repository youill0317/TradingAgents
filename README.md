...
| Regional macro | Available growth, inflation, employment, rates and selected reserves excluding gold; explicit coverage gaps | FRED; `FRED_API_KEY` needed for candidate generation |
...
Every run saves reports, `global_data.md`, `global_context.md`, raw evidence and a structured `scan.json` in its run directory. Evidence records sources, retrieval times, observation dates and availability. Missing or stale data is visible: actual collection gaps produce `DEGRADED` results, while deliberately unconfigured regional series remain explicit coverage notes. Candidate conviction is lowered only for its missing sector benchmark, not for every unrelated warning. At least one usable macro and cross-asset observation and a successful news lookup (which may return no matches) are required, alongside essential reports, sector evidence and screener evidence; a missing input family produces `INCOMPLETE` with no candidates. The `market` command exits unsuccessfully for `INCOMPLETE` runs. A successful screen with no matches is a valid empty result.

Existing provider and model settings are retained. No new data subscriptions are required; your LLM provider credentials are still necessary. With the current FRED-only macro baseline, missing FRED credentials prevent candidate generation; evidence-limited reports are still saved. Public endpoints can rate-limit or deny access, and optional source failures remain visible without inventing replacement evidence.
